...
#
# `ethos` checks a proof against a Eunoia signature; it is not a checker
# generated by eudaimonia and does not produce that framework's four verdicts.
# Here a test either prints `correct` or is rejected with an error, and each
